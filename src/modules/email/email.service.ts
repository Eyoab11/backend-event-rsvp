...
export class EmailService {
  ...
  private readonly calendarIconUrl: string;
  private readonly clockIconUrl: string;
  private readonly locationIconUrl: string;
  private readonly bowIconUrl: string;
  ...
  constructor() {
    ...
    // Self-hosted image URLs from environment variables (required)
    ...
    if (!heroImageUrl || !logoUrl || !calendarIconUrl || !clockIconUrl || !locationIconUrl || !bowIconUrl) {
      this.logger.error('Email image URLs are required but not set. Please set all EMAIL_*_URL environment variables.');
      throw new Error('Email image URLs environment variables are required');
    }

    this.heroImageUrl = heroImageUrl;
    this.logoUrl = logoUrl;
    this.calendarIconUrl = calendarIconUrl;
    this.clockIconUrl = clockIconUrl;
    this.locationIconUrl = locationIconUrl;
    this.bowIconUrl = bowIconUrl;
    
    this.resend = new Resend(apiKey);
    this.logger.log('Email service initialized with Resend API');
    this.logger.log(`Email images: Hero=${this.heroImageUrl}, Logo=${this.logoUrl}`);
  }
...
<body style="font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Arial, sans-serif; line-height: 1.5; color: #ffffff; max-width: 600px; margin: 0 auto; padding: 0; background-color: #000000;">
  ...
  <div style="background: linear-gradient(180deg, #0a0a0a 0%, #1a1a1a 100%); padding: 25px 20px;">
    ...
    <p style="color: #999999; font-size: 13px; margin: 15px 0 0 0; line-height: 1.5;">
      Questions? Contact us at <a href="mailto:info@levyeromomedia.com" style="color: #f59e0b; text-decoration: none;">info@levyeromomedia.com</a>
    </p>
...
If you have any questions, please contact us at info@levyeromomedia.com
